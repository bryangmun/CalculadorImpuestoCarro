...
public class CalculadorImpuestos {
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        if (darVehiculoActual().darPrecio() <= 30000000){
            pago=darVehiculoActual().darPrecio()*1.5/100;
        }
        else
        {
            if (darVehiculoActual().darPrecio() <=70000000){
                pago= darVehiculoActual().darPrecio()*2/100;
            }
            else {
                if (darVehiculoActual().darPrecio() <= 200000000) {
                    pago = darVehiculoActual().darPrecio() * 2.5 / 100;
                } else
                {
                    pago = darVehiculoActual().darPrecio() * 4 / 100;
                }
            }
        }
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        for (int i = 0; i < vehiculos.length; ++i){
            ...
            if (vehiculo.darPrecio()>CaroPrecio){
                CaroPrecio=vehiculo.darPrecio();
                masCaro=vehiculo;
                posVehiculoActual = i;
            }
    ...
    public Vehiculo buscarVehiculoPorMarca() {
        ...
        String marcaingresada = JOptionPane.showInputDialog("Ingrese la marca del vehículo a buscar");

        for (int i = 0; i < vehiculos.length; ++i){
            Vehiculo vehiculo = vehiculos[i];
            if (vehiculo.darMarca().equals(marcaingresada)){
                posVehiculoActual = i;
                buscado=vehiculo;
            }
    ...
    public Vehiculo buscarVehiculoPorLinea() {
        Vehiculo buscado = null;
        String lineaingresada = JOptionPane.showInputDialog("Ingrese la línea del vehículo a buscar");

        for (int i = 0; i < vehiculos.length; ++i){
            Vehiculo vehiculo = vehiculos[i];
            if (vehiculo.darLinea().equals(lineaingresada)){
                posVehiculoActual = i;
                buscado=vehiculo;
            }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for (int i = 0; i < vehiculos.length; ++i){
            Vehiculo vehiculo = vehiculos[i];
            if (Integer.valueOf(vehiculo.darAnio())<=aniomenor){
                aniomenor=Integer.valueOf(vehiculo.darAnio());
                buscado=vehiculo;
                posVehiculoActual = i;
            }
